...
//and their importance to solving problems like these. I learned how to represent a sequence of stacks as an arraylist.
...
{   
    ...
    public boolean isMatch(int dest, String symbol){
        ...
        for(int i = 0; i < RUN_LENGTH - 1; i++){
            if(symbols[i] == symbol){
                if(destStack.getTop().getSymbol() == symbols[i + 1]){
                    return true;
                }else{
                    return false;
                }
    ...
    public void doMove(String symbol, int src, int dest){
        ...
        while(!found && srcStack.size() > 0){
            ...
            if(temp.getTop().getSymbol() == symbol){
                found = true;
            }
        }
        while(temp.size() > 0){
